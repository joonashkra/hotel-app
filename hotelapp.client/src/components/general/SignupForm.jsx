...
export default function SignupForm({ toggleSignup }) {
    ...
    const signup = async (e) => {
      ...
      try {
        if(isStaff) {
          await userService.createStaff({ userName, email, phoneNumber, passwordHash, role })
          ...
          await userService.create({ userName, email, phoneNumber, passwordHash, role: '' })
          window.alert("Your account was created succesfully! Log in to continue.")
  ...
  return (
    <form className='authForm' onSubmit={signup}>
        <div className='authInput'>
            <label>Name</label>
            <input required type='text' value={userName} name='username' onChange={({ target }) => setUserName(target.value)} data-testid="username" placeholder='Type name...'/>
        </div>
        <div className='authInput'>
            <label>Email</label>
            <input required type='text' value={email} name='email' onChange={({ target }) => setEmail(target.value)} data-testid="email" placeholder='Type email...'/>
        </div>
        <div className='authInput'>
            <label>Phone number</label>
            <input required type='text' value={phoneNumber} name='phone' onChange={({ target }) => setPhoneNumber(target.value)} data-testid="phone" placeholder='Type phone number...' />
        </div>
        <div className='authInput'>
